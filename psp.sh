...
PSP_SCRIPT_PATH="$(resolve_path "${BASH_SOURCE[0]}")"
PSP_SCRIPT_DIR="$(cd -P "$(dirname "$PSP_SCRIPT_PATH")" && pwd)"
...
# Initialise ~/.psp/config.toml in one shot (--init).
init_all() {
  printf 'PSP dir: %s\n' "$PSP_DIR"
...
path_block_state() {
  local rc="$1" start='# >>> psp >>>' end='# <<< psp <<<'
  local has_start=0 has_end=0
  [[ -f "$rc" ]] || { printf 'missing\n'; return 0; }
  grep -Fx 'export PATH="$HOME/.local/bin:$PATH"' "$rc" >/dev/null 2>&1 && {
    printf 'present\n'
    return 0
  }
  grep -F "$start" "$rc" >/dev/null 2>&1 && has_start=1 || true
  grep -F "$end" "$rc" >/dev/null 2>&1 && has_end=1 || true
...
ensure_path_block() {
  ...
  case "$state" in
    present)
      ...
      ;;
    partial)
      quit "Refusing to edit $rc because it contains a partial PSP marker block"
      ;;
  esac
  mkdir -p "$(dirname "$rc")"
  touch "$rc"
...
install_symlink() {
  ...
  local link_path="${bin_dir}/psp"
  mkdir -p "$bin_dir"
  if [[ -L "$link_path" ]]; then
    if [[ "$(resolve_path "$link_path")" == "$PSP_SCRIPT_PATH" ]]; then
      printf 'Launcher already installed: %s -> %s\n' "$link_path" "$PSP_SCRIPT_PATH"
      return 0
    fi
    quit "Refusing to overwrite existing symlink: $link_path"
  fi
  if [[ -e "$link_path" ]]; then
    quit "Refusing to overwrite existing file: $link_path"
  fi
  ln -s "$PSP_SCRIPT_PATH" "$link_path"
  printf 'Installed launcher: %s -> %s\n' "$link_path" "$PSP_SCRIPT_PATH"
}

install_self() {
  printf 'Installing PSP from %s\n' "$PSP_SCRIPT_DIR"
  init_all
  install_symlink
  ensure_path_block "${HOME}/.zshrc"
  ensure_path_block "${HOME}/.bashrc"
  printf 'Install complete. Open a new shell, then run: psp --help\n'
}
...
# ── Config file (lowest priority; CLI flags override) ─────────────────────────
load_config
...
while (( $# )); do
  case "$1" in
    ...
    --history)
      ...
      cut -f5 "$PSP_DIR/history" | awk '!seen[$0]++'; exit 0 ;;
    --dry-run)       dry_run=1 ;;
...
elif [[ -z "$goal_text" && "$plan_explicit" == 0 && -z "$init_plan_path" ]]; then
  printf 'Goal: ' >&2
...
# Column width for the generation counter (e.g. " 1/10", "10/10")
gen_w=${#generations}
nm_w=$(( gen_w * 2 + 1 ))
yolo_note=""; (( yolo_mode )) && yolo_note=" | yolo: yes"
if [[ "$plan_explicit" == 1 ]]; then
  printf "| PSP %-*s | plan: %s | agent: %s | generations: %s%s |\n" \
    "$nm_w" "RUN" "$plan_display" "$agent" "$generations" "$yolo_note"
else
  printf "| PSP %-*s | goal: %s | agent: %s | generations: %s%s |\n" \
    "$nm_w" "RUN" "$goal_text" "$agent" "$generations" "$yolo_note"
fi
if [[ "$dry_run" == 1 ]]; then
...
for ((generation=1; generation<=generations; generation++)); do
  ...
  printf "| PSP %*d/%-*d | running...%s |\n" "$gen_w" "$generation" "$gen_w" "$generations" "$gen_log_note"
  gen_t=$SECONDS
  ...
  if [[ "$generation_end" == "$generation_start" ]]; then
    ...
    printf "| PSP %*d/%-*d | running → no commit | took: %ds |\n" "$gen_w" "$generation" "$gen_w" "$generations" "$took"
  else
    append_result "$generation" "committed" "$generation_end" "HEAD advanced"
    printf "| PSP %*d/%-*d | running → committed %s | took: %ds |\n" "$gen_w" "$generation" "$gen_w" "$generations" "${generation_end:0:7}" "$took"
  fi
